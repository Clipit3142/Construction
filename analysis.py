    ...
    for target in target_cols:
        ...
        for feature in feature_cols:
            ...
            for lag in range(max_lag+1):
                a = df[target].diff( 1 if feature != target else 12 ) if diff else df[target]
                corr = a.corr(df[feature].diff(1 if feature != target else 12).shift(lag) if diff else df[feature].shift(lag))
                corrs.append(corr)
...
def bar(corr_dict, max_lag, target, feature):
    fig = go.Figure(
        [go.Bar(
            x=[f"lag {l}" for l in range(max_lag + 1)],
            y=corr_dict[target][feature]
        )]
    )
    fig.update_yaxes(range=[-1, 1], zeroline=True, zerolinewidth=2, zerolinecolor="black")
...
def stl(df, cols):
    ...
    titles = [item for col in cols for item in (col, "Trend", "Seasonal", "Residual", "")]
    ...
    for i, row in enumerate(cols):
        ...
        stls[row] = {"trend": result.trend, "seasonal": result.seasonal, "resid": result.resid}
        fig.add_trace(go.Scatter(x=df.index, y=result.observed, mode="lines"), row=rows_per_group*i+1, col=1)
        fig.add_trace(go.Scatter(x=df.index, y=result.trend, mode="lines"), row=rows_per_group*i+2, col=1)
        fig.add_trace(go.Scatter(x=df.index, y=result.seasonal, mode="lines"), row=rows_per_group*i+3, col=1)
        fig.add_trace(go.Scatter(x=df.index, y=result.resid, mode="lines"), row=rows_per_group*i+4, col=1)
