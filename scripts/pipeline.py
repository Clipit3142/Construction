...
def run_pipeline():
    df = pd.read_excel(
        "data/raw/download.xls", sheet_name=2, header=138, usecols="B,D:G"
    )
    ...
    df["interest_rate"] = pd.read_excel(
        "data/raw/interest_rates.xlsx", sheet_name="Chart", header=76, usecols="B"
    )
    df["currency_exchange_rate"] = pd.read_excel(
        "data/raw/exchange_rates.xlsx", sheet_name="Chart", header=508, usecols="B"
    )
    df["economic_perception_index"] = pd.read_excel(
        "data/raw/IPEC.xlsx", sheet_name="Chart", header=2, usecols="B"
    )
    df["economic_activity_indicator"] = pd.read_excel(
        "data/raw/economic_activity.xlsx", sheet_name="Chart", header=88, usecols="B"
    )
