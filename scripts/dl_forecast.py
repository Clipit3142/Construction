...
test_data, _, _ = normalize(test_data, norm_cols, min_vals=train_min, max_vals= train_max)
...
for i, name in enumerate(cols):
    full_series = (test[:, i] * (train_max[i] - train_min[i]) + train_min[i]).numpy()
    pred_series = (test_output[:, i] * (train_max[i] - train_min[i]) + train_min[i]).numpy()

    fig.add_trace(
        go.Scatter(x=x_input, y=full_series, mode="lines", name="Full", line=dict(color="gray")),
        row=i+1, col=1
    )
    fig.add_trace(
        go.Scatter(x=x_future, y=pred_series, mode="lines", name="Pred", line=dict(color="blue")),
        row=i+1, col=1
    )
